...
export default function SearchPage() {
  ...
  return (
    <div className="flex flex-col">
      {/* Search Input */}
      <div className="sticky top-0 z-[49] bg-white dark:bg-background border-b border-sand-200 dark:border-border pt-safe">
        <div className="flex items-center gap-2 px-[var(--page-padding-x)] py-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-sand-200 bg-sand-50 dark:bg-muted py-2.5 pe-9 ps-9 text-sm placeholder:text-sand-400 dark:placeholder:text-muted-foreground focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
            />
            {query && (
              <button
                onClick={() => setQuery("")}
                className="absolute end-3 top-1/2 -translate-y-1/2 rounded-full p-0.5 hover:bg-sand-200 dark:hover:bg-muted"
              >
                <X className="h-3.5 w-3.5 text-sand-400" />
              </button>
            )}
          </div>
          <button className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-sand-200 dark:border-border hover:bg-sand-50 dark:hover:bg-muted">
              ...
              <div className="mt-2 space-y-1">
                {recentSearches.map((term) => (
                  <button
                    ...
                    className="flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-start transition-colors hover:bg-sand-50 dark:hover:bg-muted"
                  >
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {mockTrendingSearches.map((term) => (
                <Badge
                  key={term}
                  variant="secondary"
                  className="cursor-pointer px-3 py-1.5 text-xs hover:bg-sand-200 dark:hover:bg-muted"
                  onClick={() => handleSearch(term)}
                >
