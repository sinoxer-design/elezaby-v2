  ...
  if (!article) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 px-[var(--page-padding-x)] py-16">
        <p className="text-sm font-medium text-sand-600 dark:text-muted-foreground">Article not found</p>
        <Link href="/blog" className="text-sm font-semibold text-brand-500 dark:text-primary">
          Back to Blog
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col pb-8">
      ...
      <div className={cn("bg-gradient-to-br px-[var(--page-padding-x)] pb-6 pt-3 lg:px-8", article.color, article.darkColor)}>
        ...
        <div className="flex items-center justify-between">
          <Button variant="ghost" size="icon-sm" className="bg-white/50 dark:bg-black/20 backdrop-blur-sm" asChild>
            <Link href="/blog">
              <ChevronLeft className="h-5 w-5 text-sand-700 dark:text-foreground" />
            </Link>
          </Button>
          <div className="flex gap-2">
            <button className="flex h-8 w-8 items-center justify-center rounded-full bg-white/50 dark:bg-black/20 backdrop-blur-sm">
              <Share2 className="h-4 w-4 text-sand-600 dark:text-muted-foreground" />
            </button>
            <button className="flex h-8 w-8 items-center justify-center rounded-full bg-white/50 dark:bg-black/20 backdrop-blur-sm">
              <Bookmark className="h-4 w-4 text-sand-600 dark:text-muted-foreground" />
            </button>
          </div>
        </div>
        ...
        {/* Title */}
        <h1 className="mt-4 text-2xl font-bold leading-tight text-sand-800 dark:text-foreground lg:text-3xl">
          {article.title}
        </h1>
        <p className="mt-2 text-sm text-sand-600 dark:text-muted-foreground">{article.excerpt}</p>
      </div>

      {/* Author & Meta Bar */}
      <div className="flex items-center gap-3 border-b border-sand-100 dark:border-border px-[var(--page-padding-x)] py-3 lg:px-8">
        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-50 dark:bg-accent">
          <User className="h-4 w-4 text-brand-500 dark:text-accent-foreground" />
        </div>
        <div className="flex-1">
          <p className="text-sm font-semibold text-sand-700 dark:text-foreground">Dr. Sarah Ahmed</p>
          <p className="text-[11px] text-sand-400 dark:text-muted-foreground">Clinical Pharmacist</p>
        </div>
        <div className="flex items-center gap-3 text-[11px] text-sand-400 dark:text-muted-foreground">
      ...
      <div className="mx-auto w-full max-w-2xl flex flex-col gap-5 px-[var(--page-padding-x)] pt-5 lg:px-8">
        {article.body.map((paragraph, index) => (
          <p key={index} className="text-[15px] leading-[1.75] text-sand-600 dark:text-muted-foreground lg:text-base">
            {paragraph}
          </p>
        ))}
      </div>
      ...
      <div className="mx-auto w-full max-w-2xl px-[var(--page-padding-x)] mt-8 lg:px-8">
        <div className="rounded-xl bg-brand-50 dark:bg-accent p-4">
          <p className="text-sm font-semibold text-brand-800 dark:text-accent-foreground">Need advice?</p>
          <p className="mt-0.5 text-xs text-brand-600 dark:text-accent-foreground/80">
      ...
      <div className="mt-8 px-[var(--page-padding-x)] lg:px-8">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-bold text-sand-800 dark:text-foreground">Keep Reading</h2>
          <Link href="/blog" className="flex items-center gap-0.5 text-xs font-semibold text-brand-500 dark:text-primary">
        ...
        <div className="flex gap-3 overflow-x-auto scrollbar-hide md:grid md:grid-cols-3 md:overflow-visible">
          {relatedArticles.map((related) => (
            <Link
              ...
              className="w-[65%] shrink-0 overflow-hidden rounded-xl border border-sand-200 dark:border-border bg-white dark:bg-card md:w-auto"
            >
              ...
              <div className="p-3">
                ...
                <span className="mt-1.5 flex items-center gap-1 text-[11px] text-sand-400 dark:text-muted-foreground">
