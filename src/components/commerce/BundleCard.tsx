...
export function BundleCard({
  ...
  return (
    ...
    >
      <div className="flex h-full w-full flex-col overflow-hidden rounded-xl border border-sand-200 dark:border-border border-l-4 border-l-deal bg-white dark:bg-card">
        {/* Header */}
        <div className="flex items-start justify-between p-3 pb-2">
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-0.5 text-xs text-sand-500 dark:text-muted-foreground line-clamp-1">
              {bundle.description}
            </p>
          </div>
          {bundle.badge && (
            <span className="shrink-0 rounded-md bg-deal/10 px-2 py-0.5 text-[0.625rem] font-bold text-deal dark:bg-deal/20">
              {bundle.badge}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-center gap-1.5 px-3 py-2">
          {bundle.products.map((product, idx) => (
            <div key={product.productId} className="flex items-center gap-1.5">
              <div className="relative h-12 w-12 overflow-hidden rounded-full border border-sand-100 dark:border-border bg-sand-50 dark:bg-secondary">
                <Image
        ...
        <div className="px-3 pb-2">
          {bundle.products.map((product) => (
            <p
              ...
              className="text-[0.625rem] text-sand-500 dark:text-muted-foreground truncate"
            >
              {product.name}
            </p>
          ))}
        ...
        {/* Price Comparison */}
        <div className="mt-auto border-t border-sand-100 dark:border-border p-3">
