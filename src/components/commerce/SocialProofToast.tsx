  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          <div className="flex items-center gap-3 rounded-lg border border-sand-200 bg-white/95 px-4 py-2.5 shadow-elevated backdrop-blur-sm dark:border-border dark:bg-card/95">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-50">
              <ShoppingBag className="h-4 w-4 text-brand-600" />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-sand-700 dark:text-foreground">
                {message.name}
              </p>
              <p className="text-[0.625rem] text-sand-500 dark:text-muted-foreground">
                just ordered{" "}
                <span className="font-medium text-brand-600 dark:text-primary">
                  {message.product}
                </span>{" "}
                — {message.time}
              </p>
